...
// ---------------------------------------------------------
// MOTOR DE CÁLCULO DE POSICIONES (CABB - Diferencia de Gol Global)
// ---------------------------------------------------------
function calculateStandings(teams, matches) {
    ...
    // PASO 1: Calcular Récord y Puntos Globales
    matches.forEach(m => {
    ...
    // PASO 2: Ordenamiento simple y directo (Pts -> Dif Gol -> Goles a Favor)
    standings.sort((a, b) => {
        
        // 1° Criterio: Mayor cantidad de Puntos
        if (b.pts !== a.pts) return b.pts - a.pts;

        // 2° Criterio: Mayor Diferencia de Gol General (GF - GC)
        const diffA = a.pf - a.pc;
        const diffB = b.pf - b.pc;
        if (diffA !== diffB) return diffB - diffA;

        // 3° Criterio: Mayor cantidad de Goles a Favor
...
function getGlobalBestFourths() {
    ...
    // Ordenar a los mejores 4tos (Por % de puntos posibles, luego promedio de Dif Gol)
    allFourths.sort((a, b) => {
        ...
        // Si el % de puntos es igual, usamos el promedio de diferencia de gol
        const diffAvgA = a.pj > 0 ? ((a.pf - a.pc) / a.pj) : 0;
        const diffAvgB = b.pj > 0 ? ((b.pf - b.pc) / b.pj) : 0;
        
        if (Math.abs(diffAvgB - diffAvgA) > 0.0001) return diffAvgB - diffAvgA;
...
function renderStandings(teams, matches) {
    ...
    standings.forEach((t, i) => {
        ...
        // Link dinámico a la página de estadísticas
        const teamLink = `equipo.html?liga=pre_liga_metropolitana&equipo=${encodeURIComponent(t.name)}`;
